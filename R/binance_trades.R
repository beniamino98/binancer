#' Binance Aggregated Historical Trades
#'
...
#'   - `"spot"`: for endpoint [api/v3/aggTrades](https://developers.binance.com/docs/binance-spot-api-docs/rest-api/market-data-endpoints#compressedaggregate-trades-list). The ip weight is 4.
#'   - `"fapi"`: for endpoint [fapi/v1/aggTrades](https://developers.binance.com/docs/derivatives/usds-margined-futures/market-data/rest-api/Compressed-Aggregate-Trades-List). The ip weight is 20.
#'   - `"dapi"`: for endpoint [dapi/v1/aggTrades](https://developers.binance.com/docs/derivatives/coin-margined-futures/market-data/rest-api/Old-Trades-Lookup). The ip weight is 20.
#'   - `"eapi"`: for endpoint [eapi/v1/aggTrades](https://binance-docs.github.io/apidocs/voptions/en/#old-trade-lookup).
...
#' 
#' @return A \code{\link[tibble]{tibble}} with 9 columns:
...
binance_spot_trades <- function(pair, from, to, quiet = FALSE){
  ...
  while(condition){
    ...
    new_data <- binance_query(api = "spot", path = "aggTrades", query = api_query)
...
binance_fapi_trades <- function(pair, from, to, quiet = FALSE){
  ...
  while(condition){
    # GET call 
    api_query <- list(symbol = pair, startTime = start_time, endTime = NULL, limit = 1000)
    new_data <- binance_query(api = "fapi", path = "aggTrades", query = api_query)
...
binance_dapi_trades <- function(pair, from, to, quiet = FALSE){
  ...
  while(condition){
    # GET call 
    api_query <- list(symbol = pair, startTime = start_time, endTime = NULL, limit = 1000)
    new_data <- binance_query(api = "dapi", path = "aggTrades", query = api_query)
    # Break if new_data is empty 
    if (purrr::is_empty(new_data)) {
      break
    }
